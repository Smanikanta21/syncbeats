...
export default function WaitlistModal({ setShowWaitlist }: PropData) {
    ...
    const handleWaitlist = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true)
            // Simulating API call for now, or use a specific waitlist endpoint if available
    ...
    return (
        <div className="glass-card p-8 rounded-2xl w-full border-[var(--accent)]/20 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden">
            ...
            <form onSubmit={handleWaitlist} className="space-y-4 relative z-10">
                <div className="space-y-2">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-[var(--surface-2)] text-white rounded-xl border border-[var(--border-light)] focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] focus:outline-none transition-all placeholder:text-[var(--text-muted)]"
                            required
                        />
                    </div>
                </div>
                ...
                <button
                    type="submit"
                    className="w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 group mt-6 bg-[var(--accent)] hover:bg-[var(--accent)]/90 text-white transition-all shadow-[0_0_20px_var(--accent)]/20"
                    disabled={loading}
                >
                    {loading ? (
                        ...
                        <>
                            Join Waitlist <UserPlus size={18} className="group-hover:translate-x-1 transition-transform" />
                        </>
                    )}
                </button>
            </form>
