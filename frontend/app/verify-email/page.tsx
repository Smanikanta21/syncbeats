...
function VerifyEmailContent() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-black/55 p-8 sm:p-10 shadow-[0_30px_90px_rgba(0,0,0,0.5)] backdrop-blur-xl relative">
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
            {status === "loading" && <Loader2 className="h-5 w-5 text-zinc-300 animate-spin" />}
            {status === "success" && <CheckCircle2 className="h-5 w-5 text-emerald-300" />}
            {status === "error" && <AlertCircle className="h-5 w-5 text-red-300" />}
          </div>
          <div>
            <h1 className="text-2xl font-black tracking-tight text-zinc-100">Verify Your Email</h1>
            <p className="text-xs uppercase tracking-[0.24em] text-zinc-500 font-semibold">SyncBeats Account Setup</p>
          </div>
        </div>

        <p className={`mt-6 text-sm leading-relaxed ${status === "success" ? "text-emerald-300" : status === "error" ? "text-red-300" : "text-zinc-300"}`}>
          {message}
        </p>

        {status === "loading" && (
          <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.02] p-4 text-xs text-zinc-400 flex items-center gap-2">
        ...
        {status === "error" && (
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] p-4 sm:p-5">
            <label className="text-xs uppercase tracking-[0.2em] text-zinc-500 font-semibold">Resend Verification Email</label>
            <div className="mt-3 flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 bg-black/50 pl-10 pr-4 py-3 text-zinc-100 outline-none focus:border-white/30"
                />
              </div>
              <button
                type="button"
                ...
                className="h-12 rounded-xl bg-zinc-100 px-4 font-semibold text-black hover:bg-white disabled:opacity-60 disabled:cursor-not-allowed"
              >
        ...
        <div className="mt-7 flex items-center gap-4 text-sm">
          <Link href="/login" className="text-zinc-200 hover:text-white font-medium">Back to login</Link>
          <span className="text-zinc-600">•</span>
          <Link href="/verify-email-sent" className="text-zinc-400 hover:text-zinc-200">Need another link?</Link>
        </div>
      </div>
    </main>
  );
