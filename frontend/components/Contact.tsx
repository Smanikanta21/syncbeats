    ...
    <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 relative z-10">
      <div className="max-w-4xl mx-auto group">
        ...
        <motion.div
          ...
          className="glass-panel group-hover:border-white/10 p-6 sm:p-8 md:p-12 rounded-[2rem] sm:rounded-[3rem] relative overflow-hidden transition-all duration-700 bg-black/60 backdrop-blur-3xl shadow-xl group-hover:shadow-[0_0_40px_rgba(255,255,255,0.02)]"
        >
          ...
          <form className="relative z-10 space-y-6 sm:space-y-8" onSubmit={(e) => e.preventDefault()}>
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full h-14 sm:h-16 bg-zinc-200 text-black font-bold text-base sm:text-lg rounded-[1.5rem] hover:bg-white transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(255,255,255,0.05)] group/btn"
            >
              Send Message <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 group-hover/btn:translate-x-1 transition-transform text-black" />
            </motion.button>
          </form>
        </motion.div>
      </div>
