              ...
              <div className="w-full text-center mb-6 relative">
                {!isEditingProfile ? (
                  <div className="flex flex-col items-center group cursor-pointer" onClick={() => setIsEditingProfile(true)}>
                    ...
                    <p className="text-foreground/50 font-medium text-sm mt-1">{user?.email}</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-3 w-full">
                    ...
                    <div className="flex items-center gap-2 w-full justify-center">
                      <button onClick={() => { setIsEditingProfile(false); setProfileName(user?.name ?? ""); }} className="flex-1 max-w-[120px] py-2.5 rounded-xl bg-foreground/5 hover:bg-foreground/10 text-foreground font-semibold text-xs transition-all">
                        Cancel
                      </button>
            ...
            <AnimatePresence mode="popLayout">
              ...
              {activePanel === 'settings' && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className={cn(
                    "flex flex-col w-full max-w-md shrink-0 relative rounded-[2.5rem] bg-background/95 dark:bg-black/90 backdrop-blur-3xl border border-foreground/15 shadow-[0_30px_90px_rgba(0,0,0,0.6)] p-6 z-10 h-[75vh] md:h-[580px] max-h-[580px] min-h-0 overflow-hidden transition-all duration-300",
